...
@section('title', 'الخدمات والصنايعية · بنهاوي')
...
<div class="app-head" style="background: transparent;">
    ...
    <div class="title">الخدمات والصنايعية</div>
</div>

{{-- ── HERO ───────────────────────────────────────────────── --}}
<div style="padding: 4px 14px 0;">
    <div class="cat-hero" style="background: linear-gradient(135deg, #0369A1 0%, #075985 100%);">
        <div class="cat-hero-text">
            <div class="cat-hero-eyebrow">{{ $businesses->count() }} مقدّم خدمة في بنها</div>
            <h1 class="cat-hero-title">صنايعية تثق فيهم</h1>
            <p class="cat-hero-sub">سباك · كهربائي · نجار · فني تكييف · دهان — واتساب مباشر.</p>
        </div>
        <span class="cat-hero-ico">
            <x-icon name="briefcase" :size="42" stroke="white"/>
        </span>
    </div>
</div>
...
<form id="cat-form" autocomplete="off" style="padding: 14px 14px 8px;">
    <label class="field" style="margin-bottom: 8px;">
        ...
        <input type="search" id="cat-search" placeholder="ابحث عن سباك، كهربائي..." inputmode="search">
...
<div class="scroll" style="padding: 4px 14px 24px;">
    @forelse($businesses as $b)
        ...
        <a href="{{ route('business.show', $b) }}"
           ...
           data-cat="{{ Str::lower($b->category ?? '') }}">
            ...
            <div class="cat-card-thumb ph ph-{{ $b->type?->slug ?? 'service' }}">
                @if($b->logo)
    ...
    @empty
        <div style="text-align: center; padding: 40px 16px;">
            <div style="width: 80px; height: 80px; border-radius: 24px; background: rgba(14,165,233,.08); display: grid; place-items: center; margin: 0 auto 12px;">
                <x-icon name="briefcase" :size="38" stroke="#0369A1"/>
            </div>
            <div class="label-strong" style="font-size: 16px;">لسه مفيش مقدّمين خدمات</div>
            <p class="muted" style="margin-top: 6px;">كن أول صنايعي يسجّل في بنهاوي.</p>
        </div>
    @endforelse
    ...
    {{-- Register CTA --}}
    <div style="margin-top: 22px; padding: 16px; background: linear-gradient(135deg, rgba(14,165,233,.06), transparent); border: 1px dashed rgba(14,165,233,.30); border-radius: 14px; text-align: center;">
        <div style="font-weight: 800; font-size: 13.5px;">بتقدّم خدمة؟</div>
        <p style="font-size: 12px; color: var(--ink-3); margin: 4px 0 10px; line-height: 1.7;">سجّل خدمتك دلوقتي وابقى ظاهر لكل أهل بنها.</p>
        <a href="{{ route('register.step1') }}" class="btn btn-navy" style="padding: 10px 20px; font-size: 13px;">سجّل خدمتك</a>
    </div>
</div>
